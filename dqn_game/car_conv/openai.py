...
class DQN():
    ...
    def create_Q_network(self):
        # network weights
        # input layer
        s = tf.placeholder("float", [None, 10, 16, 1])
        ...
        pool_shape = h_conv3.get_shape().as_list()  # 计算全连接层空间大小
        nodes = pool_shape[1] * pool_shape[2] * pool_shape[3]  # 计算全连接层空间大小

        print("-------- nodes num: ",nodes)

        W_fc1 = weight_variable([nodes, 1024])
        ...
        h_conv3_flat = tf.reshape(h_conv3, [-1, nodes])
...
def main():
    ...
    while True:
        ...
        print('step: ', step, "   reward_num: ", reward_sum,  "epsilon:", agent.epsilon)
